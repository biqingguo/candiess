 ...
 * @version 1.0.0
 */
public class ManageMysqlEventParser extends AbstractMysqlEventParser implements CanalEventParser, CanalHASwitchable {
	...
	private MysqlConnection buildMysqlConnection(AuthenticationInfo runningInfo) {
		MysqlConnection connection = new ManageMysqlConnection(runningInfo.getAddress(), runningInfo.getUsername(),
				runningInfo.getPassword(), connectionCharsetNumber, runningInfo.getDefaultDatabaseName(), id);
	...
	protected EntryPosition findEndPositionWithMasterIdAndTimestamp(MysqlConnection connection) {
		...
		if (tableMetaTSDB != null) {
			...
			return findAsPerTimestampInSpecificLogFile(mysqlConnection, startTimestamp, endPosition,
	...
	protected EntryPosition findPositionWithMasterIdAndTimestamp(MysqlConnection connection,
		...
		if (tableMetaTSDB != null && (fixedPosition.getTimestamp() == null || fixedPosition.getTimestamp() <= 0)) {
			...
			return findAsPerTimestampInSpecificLogFile(mysqlConnection, startTimestamp, fixedPosition,
				...
				public boolean sink(LogEvent event) {
					...
					try {
						...
						if (CanalEntry.EntryType.TRANSACTIONBEGIN.equals(entry.getEntryType())
							...
							logger.debug("compare exit condition:{},{},{}, startTimestamp={}...",
